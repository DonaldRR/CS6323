...
void main(){
    ...
    vec3 normal_v = normalize(vec3(model_inv_t * vec4(normal, 1.0f)));
//    vec3 normal_v = normalize(normal);
    vec3 view_dir = normalize(camera_pos - frag_pos);
    
//    float dist_x = 5.0f - frag_pos[0], dist_y = 5.0f - frag_pos[1], dist_z = 10.0f - frag_pos[2];
//    float dist = sqrt(dist_x * dist_x + dist_y * dist_y + dist_z * dist_z);
    float cos_dir = dot(normal_v, view_dir);
    float lightness = 0.5 * (1 + cos_dir);
    final_color = m_object.object_color * 0.2;
    final_color = m_object.object_color * 0.8 * lightness;

//    final_color = 0.5 * m_object.object_color;
//
//    if(dir_light.status){
