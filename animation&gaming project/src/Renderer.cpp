...
Scheduler* Renderer::m_scheduler = new Scheduler();
...
//
//    int increment = 10;
//    gui_1->addGroup("Ratations");
//    nanogui::TextBox *widget = gui_1->addVariable("X", m_animation->x_rotation);
//    widget->setSpinnable(true);
//    gui_1->addVariable("Y", m_animation->y_rotation)->setSpinnable(true);
//    gui_1->addVariable("Z", m_animation->z_rotation)->setSpinnable(true);
...
{
    Shader m_shader = Shader("/Users/donald/Documents/One Drive/OneDrive - The University of Texas at Dallas/AnG/animation&gaming project/animation&gaming project/shaders/basic.vert", "/Users/donald/Documents/One Drive/OneDrive - The University of Texas at Dallas/AnG/animation&gaming project/animation&gaming project/shaders/basic.frag");
...
void Renderer::load_models()
{
    ...
    Object ball_object(BALL);
    ball_object.obj_color = glm::vec4(0.5f, 0.5f, 0.5f, 0.2f);
    ball_object.obj_name = "ball";
    
    Object plane_object(PLANE);
    plane_object.obj_color = glm::vec4(0.5f, 0.5f, 0.5f, 0.2f);
    plane_object.obj_name = "plane";
    
    bind_vaovbo(ball_object);
...
void Renderer::draw_scene(Shader& shader)
{
    ...
    std::vector<Model*> models = m_scheduler->get_models();
    
    for (auto model : models ) {
        glUniformMatrix4fv(glGetUniformLocation(shader.program, "model_inv_t"), 1, GL_FALSE, glm::value_ptr(glm::transpose(glm::inverse(model->get_model_mat()))));
        glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(model->get_model_mat()));
...
void Renderer::draw_object(Shader& shader, Model* model)
{
    glBindVertexArray(model->get_object()->vao);

    glUniform3f(glGetUniformLocation(shader.program, "m_object.object_color"), model->get_model_color()[0], model->get_model_color()[1], model->get_model_color()[2]);
    glUniform1f(glGetUniformLocation(shader.program, "m_object.shininess"), model->get_object()->shininess);

    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    glDrawArrays(GL_TRIANGLES, 0, model->get_object()->vao_vertices.size());
    glBindVertexArray(0);
}
